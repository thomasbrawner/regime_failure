...
class SpaceTimeLagger(object):
    ...
    def evaluate_event_window(self, dep_var, lag_var):
        events = self.gwf.query('{0} == 1'.format(lag_var))  # events of interest
        if dep_var == 'failure':
            ...
            self.gwf['dateInt'] = pd.to_datetime(revised_date).apply(lambda x: x.date().toordinal()) 
            expanded_dates = list(product(self.gwf['dateInt'], events['dateInt']))
        ...
        days = pd.merge(days, self.gwf[['cowcode', 'year']], left_on='idx_target', right_index=True, how='left')
    ...
    def generate_lags(self, distance_weights, prefix): 
        ...
        dist_lag.rename(columns = {'cowcode_target' : 'cowcode', 'weight' : prefix + '_lag'}, inplace=True)
